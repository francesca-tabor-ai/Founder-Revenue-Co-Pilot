...
export default function AdminCustomersPage() {
  ...
  const fetchData = async () => {
    const [custRes, orgRes] = await Promise.all([fetch("/api/admin/customers"), fetch("/api/admin/organizations")]);
    const [custData, orgData] = await Promise.all([custRes.json(), orgRes.json()]);
    setItems(Array.isArray(custData) ? custData : []);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const body = { organizationId: data.get("organizationId"), email: data.get("email"), name: data.get("name") || undefined };
    const url = modal === "create" ? "/api/admin/customers" : `/api/admin/customers/${(modal as { id: string }).id}`;
      ...
      {loading ? <p className="text-zinc-400">Loading...</p> : (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {items.map((c) => (
                <tr key={String(c.id)} className="border-b border-zinc-800/50">
                  <td className="px-4 py-3 text-white">{String(c.email)}</td>
                  <td className="px-4 py-3 text-white">{String(c.name || "—")}</td>
                  <td className="px-4 py-3 text-white">{(c.organization as { name?: string })?.name ?? "—"}</td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => setModal(c)} className="text-emerald-400 hover:underline mr-3">Edit</button>
                    <button onClick={() => handleDelete(String(c.id))} className="text-red-400 hover:underline">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={() => setModal(null)}>
          <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-white mb-4">{modal === "create" ? "Create Customer" : "Edit"}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div><label className="block text-zinc-400 text-sm mb-1">Email</label>
                <input name="email" type="email" required defaultValue={modal !== "create" ? String((modal as { email: string }).email) : ""}
              ...
              <div><label className="block text-zinc-400 text-sm mb-1">Name</label>
                <input name="name" defaultValue={modal !== "create" ? String((modal as { name?: string }).name ?? "") : ""}
                  className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-white" /></div>
